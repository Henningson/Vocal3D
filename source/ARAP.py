...
class ARAP:
    def __init__( self, points: np.ndarray, tris: np.ndarray, anchors: List[int], anchor_weight: Optional[float]=10.0, L: Optional[sparse.csc_matrix]=None ):
        ...
        #self._L = np.nan_to_num(self._L.todense())
        #self._wgts = np.nan_to_num(self._wgts)
        ...
        for i in anchors:
            E[i,i] = 1.0
        E = E.tocsc()
        test = self._L.T@self._L + self._anc_wgt*E.T@E
        test = np.nan_to_num(test) + 0.01 * np.eye(test.shape[0], test.shape[1])
        self._solver = spla.factorized(test)
